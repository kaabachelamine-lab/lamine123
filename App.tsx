...
import GeneratorScreen from './components/ImageUploader'; // Re-using ImageUploader for the new Generator Screen
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (appMode === 'start') {
      return <StartScreen onFileSelect={handleFileSelect} onStartGenerate={() => setAppMode('generate')} />;
    }

    if (appMode === 'generate') {
      return <GeneratorScreen onImageGenerated={handleImageUpload} onBack={() => setAppMode('start')} />;
    }
    
    if (appMode === 'edit' && currentImageUrl) {
        const imageDisplay = (
    ...
    // Fallback to start screen if app state is inconsistent
    return <StartScreen onFileSelect={handleFileSelect} onStartGenerate={() => setAppMode('generate')} />;
  };
  ...
  return (
    <div className="min-h-screen text-gray-100 flex flex-col">
      <Header />
      <main className={`flex-grow w-full max-w-[1600px] mx-auto p-4 md:p-8 flex justify-center ${appMode === 'edit' ? 'items-start' : 'items-center'}`}>
        {renderContent()}
      </main>
    </div>
  );
